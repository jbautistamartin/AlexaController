...
namespace AlexaController.Helpers
{
        ...
        private static readonly List<string> Services = new()
    {
        ...
        private static readonly HashSet<string> DisabledServices = new();

        // Método para desactivar los servicios
        public static void DisableServices()
        {
            Console.WriteLine("Desactivando servicios innecesarios...");
            foreach (var serviceName in Services)
            {
                try
                {
                    ...
                    if (service.Status == ServiceControllerStatus.Running)
                    {
                        ...
                        Console.WriteLine($"Servicio '{serviceName}' desactivado.");
                    }
                    else
                    {
                        Console.WriteLine($"Servicio '{serviceName}' ya estaba detenido.");
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Error al desactivar el servicio '{serviceName}': {ex.Message}");
                }
            }
        }
            ...
            foreach (var serviceName in DisabledServices)
            {
                try
                {
                    ...
                    if (service.Status == ServiceControllerStatus.Stopped)
                    {
                        ...
                        Console.WriteLine($"Servicio '{serviceName}' reactivado.");
                    }
                    else
                    {
                        Console.WriteLine($"Servicio '{serviceName}' ya estaba en ejecución.");
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Error al reactivar el servicio '{serviceName}': {ex.Message}");
                }
            }
